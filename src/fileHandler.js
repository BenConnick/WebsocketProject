...
const loadDirectoryIntoDictionary = (dic, extension) => {
  ...
  fileNames.forEach(function(name) {
    ...
    dic[`/${name}`] = fs.readFileSync(`${__dirname}/../public/${extension}/${name}`);
  });
}
...
const serveScript = (scriptName, response) => {
  ...
  // undefined error
  if (!requestedScript) { 
    throw `file "${scriptName}" was not loaded`; 
    return; 
  }; 
  // write out
...
const serveImage = (imageName, response) => {
  ...
  // undefined error
  if (!image) { 
    throw `file "${imageName}" was not loaded`; 
    return; 
  }; 
  // write out
