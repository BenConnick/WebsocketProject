...
const createRandomEnemy = (lvlMin, lvlMax) => {
	...
	let a = idxMin + Math.floor(Math.random() * (idxMax - idxMin + 1));
	console.log("a: " + a);
...
class Enemy extends Character {
	...
	seekClosestPlayer() {
		...
		let room = currentFloor.rooms[getRoomIdxFromTile(currentFloor.rooms, this.x, this.y)];
		let minDist = 1000000;
		...
		players.forEach(function (p) {
			...
			if (currentFloor.rooms[getRoomIdxFromTile(currentFloor.rooms, p.x, p.y)] == room) {
				if (!playerAlreadyFoundInSameRoom) {
					target = p;
					let distSq = (p.x - e.x) * (p.x - e.x) + (p.y - e.y) * (p.y - e.y);
					minDist = distSq;
				...
				if (!playerAlreadyFoundInSameRoom) {
					let distSq = (p.x - this.x) * (p.x - this.x) + (p.y - this.y) * (p.y - this.y);
					if (distSq < minDist) {
						minDist = distSq;
						target = p;
					}
